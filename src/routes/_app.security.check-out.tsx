...
export const Route = createFileRoute("/_app/security/check-out")({
...
function CheckOutPage() {
  ...
  const mutation = useMutation({
    mutationFn: (qrToken: string) =>
      gateScansApi.checkOut({ qrToken, gateId: 1 }),
    onSuccess: (data) => {
      setResult({ success: true, message: data.message, checkOutTime: data.checkOutTime });
      setScanning(false);
    },
    onError: (err: any) => {
      setResult({
        success: false,
        message: err?.response?.data || "Check-out failed.",
      });
      setScanning(false);
    },
  });
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6 p-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Scan Exit</h1>
        <p className="text-sm text-muted-foreground">Scan visitor QR pass to check out.</p>
      </div>
          ...
          {result && (
            <div className={`flex flex-col items-center gap-3 rounded-xl p-6 text-center ${
              ...
              <p className="text-lg font-semibold">
                {result.success ? "Checked Out!" : "Failed"}
              </p>
              <p className="text-sm text-muted-foreground">{result.message}</p>
              {result.checkOutTime && (
                <p className="text-xs text-muted-foreground">
                  Time: {new Date(result.checkOutTime).toLocaleTimeString()}
                </p>
              )}
            </div>
          )}
